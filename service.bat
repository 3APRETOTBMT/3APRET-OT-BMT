...
set "3APRETOTBMT-0.1beta"

:: External commands
if "%~1"=="3APRETOTBMT Проверка" (
    call :test_service 3APRETOTBMT
    exit /b
)

if "%~1"=="проверка обновлений" (
    call :service_check_updates soft
    exit /b
)

if "%1"=="admin" (
    echo "Запускался с правами администратора"
) else (
...
echo =======================
echo 1. Ustanovka servisa
echo 2. Udalit' sluzhby
echo 3. Prover'te sostojanie obsluzhivanija
echo 4. Zapustite diagnostiku
echo 5. Proverjat' obnovlenija
echo 0.Vyhod
...
tasklist /FI "IMAGENAME eq 3APRETOTBMT.exe" | find /I "3APRETOTBMT.exe" > nul
if !errorlevel!==0 (
...
for /f "tokens=*" %%a in ('type "!selectedFile!"') do (
    ...
    echo !line! | findstr /i "%BIN%3APRETOTBMT.exe" >nul
    if not errorlevel 1 (
        set "capture=1"
    )
    ...
    if !capture!==1 (
        if not defined args (
            set "line=!line:*%BIN%3APRETOTBMT.exe"=!"
        )
...
sc create %SRVCNAME% binPath= "\"%BIN_PATH%3APRETOTBMT.exe\" %ARGS%" DisplayName= "3APRETOTBMT" start= auto
sc description %SRVCNAME% "3APRETOTBMT 0.1beta"
sc start %SRVCNAME%
